  ...
  return (
    <div className="sidebar-options-container">
      <Link to="/" type="button" onClick={() => handleClick('home')} className={handleChange(value, 'home', 'button')}>
          <HomeOutlinedIcon className={handleChange(value, 'home', 'icon')} fontSize="large"/>
      ...
      <Link to="/dashboard" type="button" onClick={() => handleClick('dashboard')} className={handleChange(value, 'dashboard', 'button')}>
        <DashboardOutlinedIcon className={handleChange(value, 'dashboard', 'icon')} fontSize="large"/>
        <h1 className={handleChange(value, 'dashboard', 'title')}>Dashboard</h1>
      </Link>
      <Link to="/bills" type="button" onClick={() => handleClick('bill')} className={handleChange(value, 'bill', 'button')}>
        <LocalAtmOutlinedIcon className={handleChange(value, 'bill', 'icon')} fontSize="large"/>
      ...
      <Link to="/debts" type="button" onClick={() => handleClick('debt')} className={handleChange(value, 'debt', 'button')}>
        <GavelOutlinedIcon className={handleChange(value, 'debt', 'icon')} fontSize="large"/>
      ...
      <Link to="/account/settings" type="button" onClick={() => handleClick('settings')} className={handleChange(value, 'settings', 'button')}>
        <SettingsOutlinedIcon className={handleChange(value, 'settings', 'icon')} fontSize="large"/>
